...
public struct JamQuad {
	...
	public JamQuad(int _submeshIndex ,float _Width, float _Height, Vector3 _position, float _VoxelSpacing, VoxelPos normDirection, ref int _faceCount, VoxelPos _StartPos, ref VoxelSystemChunkGreedy _Chunk)
	{
		...
		if(normDirection.x == 1)
		{
            ...
            UVs[0] = new Vector2((_StartPos.y + cPos.y * ChunkSize.y * VSpace)/(ChunkSize.y * SystemSize.y * VSpace),               //0.0
                                 (_StartPos.z + cPos.z * ChunkSize.z * VSpace)/(ChunkSize.z * SystemSize.z * VSpace));           //0.0
            
            UVs[3] = new Vector2((_StartPos.y + _Height + cPos.y * ChunkSize.y * VSpace)/(ChunkSize.y * SystemSize.y * VSpace),  //1.0
                                 (_StartPos.z + cPos.z * ChunkSize.z * VSpace)/(ChunkSize.z * SystemSize.z * VSpace));           //0.0
            
            
            UVs[2] = new Vector2((_StartPos.y + _Height  + cPos.y * ChunkSize.y * VSpace)/(ChunkSize.y * SystemSize.y * VSpace),  //1.0
                                 (_StartPos.z + _Width + cPos.z * ChunkSize.z * VSpace)/(ChunkSize.z * SystemSize.z * VSpace));//1.0
            
            UVs[1] = new Vector2((_StartPos.y + cPos.y * ChunkSize.y * VSpace)/(ChunkSize.y * SystemSize.y * VSpace),              //0.0    
                                 (_StartPos.z + _Width + cPos.z * ChunkSize.z * VSpace)/(ChunkSize.z * SystemSize.z * VSpace)); //1.0
		...
		else if(normDirection.z == -1)
		{
			...
			triangles[1].verts[2] = 3 + _faceCount*4;
			UVs[0] = new Vector2((_StartPos.x + cPos.x * ChunkSize.x * VSpace)/(ChunkSize.x * SystemSize.x * VSpace),			   //0.0
			                     (_StartPos.y + cPos.y * ChunkSize.y * VSpace)/(ChunkSize.y * SystemSize.y * VSpace));           //0.0
			...
			UVs[2] = new Vector2((_StartPos.x + _Height  + cPos.x * ChunkSize.x * VSpace)/(ChunkSize.x * SystemSize.x * VSpace),  //1.0
			                     (_StartPos.y + _Width + cPos.y * ChunkSize.y * VSpace)/(ChunkSize.y * SystemSize.y * VSpace));//1.0
