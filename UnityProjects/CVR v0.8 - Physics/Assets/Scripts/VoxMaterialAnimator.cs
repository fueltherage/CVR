...
public class VoxMaterialAnimator : MonoBehaviour {
	...
	void Update () {
		for (int i = 1; i <= numberOfMats; i++) {
			Color color;
			color.a = colors[i-1].a * (Mathf.Sin( 4*i * animationLoopTime / numberOfMats + Time.time )+2)*0.5f;
			color.r = colors[i-1].r * (Mathf.Sin( 4*i * animationLoopTime / numberOfMats + Time.time )+2)*0.5f;
			color.g = colors[i-1].g * (Mathf.Sin( 4*i * animationLoopTime / numberOfMats + Time.time )+2)*0.5f;
			color.b = colors[i-1].b * (Mathf.Sin( 4*i * animationLoopTime / numberOfMats + Time.time )+2)*0.5f;
