...
public class VoxelThreads : MonoBehaviour {
	...
	private static Queue<VoxelSystemChunkGreedy> p1Chunks = new Queue<VoxelSystemChunkGreedy>();
	private static Queue<VoxelSystemChunkGreedy> p2Chunks = new Queue<VoxelSystemChunkGreedy>();
	private static Queue<VoxelSystemChunkGreedy> p3Chunks = new Queue<VoxelSystemChunkGreedy>();
...
//		}
		...
		for (int i = 0; i < chunksGenerating.Count; i++) {
			...
			{
				QueueOnMainThread(chunksGenerating[i].GenerateThisMesh);
		...
		while (p1Chunks.Count!=0 && chunksGenerating.Count < MaxVoxelMeshUpdateThreads)
		{
			...
			if(!p1Chunks.Peek().needsUpdating)
			{
				p1Chunks.Dequeue();
				continue;
			}
			if(!chunksGenerating.Contains(p1Chunks.Peek()))
			{
				...
				RunAsync(chunksGenerating[chunksGenerating.Count-1].UpdateMesh);
		...
		{
			if(!p2Chunks.Peek().needsUpdating)
			{
				p2Chunks.Dequeue();
				continue;
			}
		...
		{
			if(!p3Chunks.Peek().needsUpdating)
			{
				p3Chunks.Dequeue();
				continue;
			}
	...
	public static void QueueVoxMeshUpdate(ref VoxelSystemChunkGreedy chunk, int priority)
	{
		...
		case 2:
			...
			p2Chunks.Enqueue(chunk);
			//else Debug.Log("Threading Warning: Chunk - "+chunk.chunkPos.ToString() + " attempted to queue itself more then once.");
			break;
		case 3:
			if(!p3Chunks.Contains(chunk))
			p3Chunks.Enqueue(chunk);
			//else Debug.Log("Threading Warning: Chunk - "+chunk.chunkPos.ToString() + " attempted to queue itself more then once.");
			break;
		}
	}
