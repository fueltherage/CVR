...
public class VoxelSystem : MonoBehaviour {
	...
	void Start () {
		...
		VoxelSystem thisSystem = GetComponent<VoxelSystem>();
		...
		for (int x =0; x < XSize; x++){
			for (int y =0; y < YSize; y++){
				for (int z =0; z < ZSize; z++){
					...
					chunks_vcs[x,y,z].systemParent = thisSystem;
...
//		                           VoxelSpacing);
	...
	public void AddVoxel(int x, int y, int z, bool update, int type)
	{
		...
		if(chunkPos.x != XSize && chunkPos.y != YSize && chunkPos.z != ZSize)
		{
			...
			if(chunkPos.x >= XSize || chunkPos.y >= YSize || chunkPos.z >= ZSize)
			{
				Debug.Log("Voxel was placed out of range");
			}
			if(update)
			{
				chunks_vcs[chunkPos.x, chunkPos.y, chunkPos.z].UpdateMesh ();
				chunks_vcs[chunkPos.x, chunkPos.y, chunkPos.z].blocks[voxPos.x,voxPos.y,voxPos.z].neighbours.ForceUpdateNeighbours();
			}else 
			{
				chunks_vcs[chunkPos.x, chunkPos.y, chunkPos.z].neighbours.UpdateNeighbourMeshes();
