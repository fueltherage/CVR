...
public class VoxelSystemChunkGreedy : VoxelChunkGreedy {
	...
	void Update()
	{
		chunkPosEditor.x = chunkPos.x;
		chunkPosEditor.y = chunkPos.y;
		chunkPosEditor.z = chunkPos.z;
//        if (!Generating)
//            if (MeshBaked && needsUpdating)
//                GenerateMesh();
	...
	protected override void GenerateMesh()
	{
		...
		vmesh.Clear();
		Profiler.EndSample();
		Profiler.BeginSample("Quad generation");
...
//			Profiler.EndSample();
	...
	private int [] SubMeshTrianglesCPY(int _submesh, ref List<Triangle> _triangles, ref List<int> _trIndex)
	{
		int [] rawTri = new int[_triangles.Count*3];
		for (int i = 0; i < _triangles.Count; i++)
		{
			if(_trIndex.Count ==0)
			{
