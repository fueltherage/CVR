...
public class VoxelSystemGreedy : MonoBehaviour {
	...
	public void Init()
	{
		//VoxelThreads trash = VoxelThreads.Current;
		//Create a new empty chunk for this system and sync it's size
		if(PreMade)
			EmptyChunk = transform.FindChild("EmptyChunk").gameObject;
		else
		{
		...
		for (int x =0; x < XSize; x++){
			for (int y =0; y < YSize; y++){
				for (int z =0; z < ZSize; z++){
					if(PreMade)
						chunks[x,y,z] = transform.FindChild("VoxelSystemChunk "+x+" "+y+" "+z).gameObject;
					else
						chunks[x,y,z] = Instantiate(VoxelChunkGO, this.transform.position, Quaternion.identity) as GameObject;
					chunks[x,y,z].name = "VoxelSystemChunk "+x+" "+y+" "+z;
	...
	public void CopySystem(VoxelSystemGreedy _system)
	{
		XSize = _system.XSize;
		YSize = _system.YSize;
		ZSize = _system.ZSize;
		ChunkSizeX = _system.ChunkSizeX;
		ChunkSizeY = _system.ChunkSizeY;
		ChunkSizeZ = _system.ChunkSizeZ;
		VoxelSpacing = _system.VoxelSpacing;
		chunks = _system.chunks;
		chunks_vcs = _system.chunks_vcs;
	}
		...
		for(int x = 0; x < XSize; x++)
		{
			for(int y = 0; y < YSize; y++)
			{
				for(int z = 0; z < ZSize; z++)
				{
					...
					chunks_vcs[x,y,z].GenerateThisMesh();
				}
			}
		}
	}
