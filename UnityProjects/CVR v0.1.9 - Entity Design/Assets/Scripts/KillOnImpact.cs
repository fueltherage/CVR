        ...
        if(hit.gameObject.tag == "Chunk")
        {
            ...
            if(Physics.Raycast(transform.position, rigidbody.velocity.normalized, out rayhit, 5, mask))
            {
                ...
                if(rayhit.rigidbody!=null)
                {
                    ...
                    if(hit.transform.parent.rigidbody != null)
					{
						Vector3 dif = transform.position - hit.transform.position;
                    	hit.transform.parent.rigidbody.AddForceAtPosition(rigidbody.velocity * rigidbody.mass, transform.position + new Vector3(0.0f,Random.Range(-20,20)/10.0f,0.0f));
					}
                }
